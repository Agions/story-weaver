---
title: Docker 开发环境
description: Story Weaver Docker 开发环境（可选）：容器化前端 + 跨平台编译
category: deployment
version: '>=3.0'
---
...
> ⚠️ **注意**：Story Weaver v2.2.3 是 **Tauri 桌面应用**，**生产部署不需要 Docker**。
> 本文档仅介绍**开发环境容器化**（可选）。
...
```bash
docker build -t Story Weaver-dev -f Dockerfile.dev .
docker run -it --rm -v $(pwd):/app -p 5173:5173 Story Weaver-dev
```
