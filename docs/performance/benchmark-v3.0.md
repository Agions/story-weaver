---
title: v3.0 性能基准
description: v3.0 拆分重构后性能基准：44 大文件 → 195+ 子模块、47 个代码分割 chunk、零调用方影响、bundle/流水线/UI 实测
category: performance
version: 3.0
---

# frame-fab v3.0 性能基准报告

> 测量日期: 2026-06-10
> 测量环境: Tauri 2.1 + React 18 + TypeScript 5 + Node.js 20.x
> 测量范围: v3.0 拆分重构后（commit HEAD）
...
> ⚠️ v3.0 文件数增多 = 拆分粒度细（每文件 < 300 行）+ 测试覆盖更全。
...
> 💡 v3.0 流水线多了一步 `export`（之前合并在 composition），单步耗时略增 5-20%，
...
> v3.0 服务按 7 大领域重组后，**首次查找开销略增**（多走一层 `index.ts`），
> 但运行时缓存后**几乎无差异**。
...
> 💡 v3.0 路由级 code-splitting（47 个 chunks）让首屏只加载必要代码，
> LCP 普遍**降低 10-20%**。
...
> 💡 v3.0 测试拆分到 79 个独立 suite，**并行度更高**（jest 8 worker），
> 总时长**降低 17%**。
...
- [v2.2.0 基准报告](./benchmark-v2.2.0.md) — 历史对比
