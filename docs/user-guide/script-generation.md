...
> 本文档介绍 FrameForge 中的脚本生成功能，包括视频剧本的结构、如何生成结构化脚本以及脚本审核机制。
...
脚本生成（Script Generation）是 FrameForge 工作流中的核心步骤之一。在完成故事分析后，系统会将原始文本转换为结构化的视频剧本，为后续的角色设计和分镜生成提供清晰的执行蓝图。
...
A: 可以。FrameForge 支持将剧本导出为 JSON、Markdown、YAML、Fountain（专业剧本格式）等。
