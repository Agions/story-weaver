---
title: 脚本生成
description: Story Weaver 视频剧本自动生成：结构化剧本、场景拆分、Self-Review 审核
category: user-guide
version: '>=3.0'
---
...
> 本文档介绍 Story Weaver 中的脚本生成功能：把原始小说/剧本转换为结构化的视频剧本。

---

## 概述

脚本生成（Script Generation）是 Story Weaver 工作流中的核心步骤之一。在完成故事分析后，系统会将原始文本转换为结构化的视频剧本，为后续的角色设计和分镜生成提供清晰的执行蓝图。
...
A: 可以。Story Weaver 支持将剧本导出为 JSON、Markdown、YAML、Fountain（专业剧本格式）等。
