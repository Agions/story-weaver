...
class RenderQueueService {
  ...
  private buildFallbackImage(title: string): string {
    // Sanitize title to prevent XSS in SVG
    const sanitizeForSvg = (str: string): string => {
      return str
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
    ...
    const svg = `<svg xmlns='http://www.w3.org/2000/svg' width='1024' height='1024'>
      ...
      <text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' fill='white' font-size='46' font-family='sans-serif'>${sanitizeForSvg(title)}</text>
    </svg>`;
