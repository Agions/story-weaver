 ...
 *
 * 内部按职责拆分到 features/video-export/services/ 下的 sibling 文件：
 * - video-constants.ts                : 全部常量 / 字典
 * - ffmpeg-command-builder.ts (core)  : 流式 builder
...
} from '@/services/media/video-constants';
import {
  exportVideo,
  clipVideo,
  mergeVideos,
  ...
  type SubtitleStyle,
} from '@/services/media/video-ffmpeg-operations';
import { extractVideoInfoFromFile } from '@/services/media/video-info-extractor';
...
class VideoService {
  ...
  async analyzeVideo(videoInfo: VideoInfo): Promise<VideoAnalysis> {
    ...
    return {
      ...
      objects: [],
      emotions: [],
      summary: `Video duration ${formatDuration(videoInfo.duration!)}, resolution ${videoInfo!.width}x${videoInfo!.height}, contains ${scenes.length} scenes.`,
