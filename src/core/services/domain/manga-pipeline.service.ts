...
} from '@/core/services/audio/lip-sync.service';
  ...
  async generateFromNovel(
    ...
    try {
          ...
          pipelineScenes.forEach((scene, i) => {
            if (scene.subtitles) {
              scene.subtitles.subtitles.forEach((sub) => {
                allSubtitles.subtitles.push({
                  ...sub,
                  startTime: sub.startTime + i * 5,
                  endTime: sub.endTime + i * 5,
                });
              });
            }
          });
      ...
      return {
        ...
        finalVideoUrl: pipelineScenes[0]?.finalVideoUrl,
  ...
  async generateFromImages(
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      const videoResult = await generateVideo(prompt, {
        ...this.config.video,
        model: this.config.video?.model || 'seedance-2.0',
        referenceImage: url,
