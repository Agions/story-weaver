...
export class CharacterService {
  ...
  bulkCreate(
    ...
    const newCharacters = characters.map((data) => ({
      ...
      consistency: {
        seed:
          (data.consistency as CharacterConsistency | undefined)?.seed ??
  ...
  static validate(character: Partial<Character>): string[] {
    ...
    if (!character.appearance) {
      ...
      if (
        !(character.appearance as CharacterAppearance).age ||
        Number((character.appearance as CharacterAppearance).age) < 1 ||
        Number((character.appearance as CharacterAppearance).age) > 120
      ) {
