...
const REVIEW_EXPORT_ACTIVITY_KEY = 'panel-flow_review_export_activities';
...
class ReviewExportService {
  ...
  toMarkdown(input: ReviewExportInput): string {
    ...
    lines.push('# panel-flow AI Review Export');
    lines.push('');
    ...
    lines.push(
      `- Today: $${input.costStats.today.toFixed(4)} | This Week: $${input.costStats.thisWeek.toFixed(4)} | This Month: $${input.costStats.thisMonth.toFixed(4)}`
    );
    lines.push('');
